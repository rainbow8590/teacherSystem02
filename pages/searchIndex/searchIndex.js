...
Page({
    ...
    wx.getSystemInfo({
      success: function(res) {
        that.setData({
          ...
          resultH: res.windowHeight - (174+110)/res.pixelRatio})
      }
    });
  ...
  searchStudent: function(){
    ...
    if(sName){
      ...
      var query1 = 'appid=web&nClassYear='+ year +'&nSemester='+nSemester+'&PageIndex=1&PageSize=1000&sPhone='+ sPhone +'&sStudentName='+ sName +'&timestamp='+stamp+'&token='+token;
      var query2 = 'appid=web&nclassyear='+ year +'&nsemester='+nSemester+'&pageindex=1&pagesize=1000&sphone='+ sPhone +'&sStudentName='+ sName +'&timestamp='+stamp+'&token='+token+'test';
    }else{
      wx.showModal({
          title: '提示',
          content: '请输入学生姓名或手机号',
